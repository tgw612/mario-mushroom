...
核心
1.存储结构
传统RDBMS用B树或B+树作为数据的存储结构
HBase用LSM树作为存储结构,其使用算法对索引变更进行延迟及批量处理,并用类似归并排序的方式
磁盘的使用方式 RDBMS使用寻道,而LSM树属于传输型
2.持久化实现
3.预写日志
RegionServer在未收集到足够数据flush到磁盘之前,会一直把它存到内存中.但内存数据容易丢失,
Hbase采用WAL策咯:每次更新之前,数据先写到Hlog中,
4.写入和查询流程
5.数据备份及压缩