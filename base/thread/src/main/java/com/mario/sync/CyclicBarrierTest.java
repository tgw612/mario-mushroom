 ...
 * <p>
 * 在所有的屏障都到达之后，会启动一个线程来运行这里面的代码。 相比CountDownLatch，CyclicBarrier是可以被循环使用的，而且遇到线程中断等情况时，还可以利用reset()方法，重置计数器，
    ...
    for (int i = 0; i < 5; i++) {
      new Thread(new Runnable() {
        ...
        public void run() {
          ...
          System.out.println(
              Thread.currentThread().getName() + " " + new Date() + " run, sleep " + secs
                  + " secs");
          try {
