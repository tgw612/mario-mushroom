 ...
 *
 * 可重入锁中可重入表示的意义在于对于同一个线程，可以继续调用加锁的方法，
 * 而不会被挂起。可重入锁内部维护一个计数器，对于同一个线程调用lock方法，计数器+1，调用unlock方法，计数器-1。
 */
...
//    }
    ...
    /**
     * 举个例子再次说明一下可重入的意思，在一个加锁方法execute中调用另外一个加锁方法anotherLock并不会被挂起，
     * 可以直接调用(调用execute方法时计数器+1，然后内部又调用了anotherLock方法，计数器+1，变成了2)：
     */
    public void execute() {
        lock.lock();
        try {
